...
mod tests {
  ...
  "resources": {{
    ...
    fn load_rejects_unknown_fields() {
        let dir = std::env::temp_dir().join("ring_host_tauri_config_unknown");
    ...
    fn validate_rejects_empty_start_script_path() {
        let root = std::env::temp_dir().join("ring_host_tauri_config_validate");
        let assets = root.join("assets");
        std::fs::create_dir_all(&assets).unwrap();

        let mut config = AppConfig::default();
        config.assets_root = assets.clone();
        config.saves_dir = root.join("saves");
        config.manifest_path = "manifest.json".to_string();
        config.default_font = "fonts/simhei.ttf".to_string();
        config.start_script_path.clear();
        config.debug = DebugConfig::default();
        config.audio = AudioConfig::default();
        config.resources = ResourceConfig::default();
