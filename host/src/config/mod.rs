...
mod tests {
    ...
    fn make_valid_fs_config() -> (AppConfig, TempPath) {
        ...
        config.asset_source = AssetSourceType::Fs;
        config.assets_root = root.path.clone();
        config.start_script_path = "start.md".to_string();
        (config, root)
    }
    ...
    fn test_load_branches() {
        ...
        assert_eq!(loaded_missing.window.width, AppConfig::default().window.width);
        ...
        assert_eq!(loaded_parse_err.window.height, AppConfig::default().window.height);

        let read_err_dir = TempPath::dir("ring-config-test-read-error-dir");
        let loaded_read_err = AppConfig::load(read_err_dir.as_path());
        assert_eq!(loaded_read_err.window.title, AppConfig::default().window.title);
    ...
    fn test_manifest_full_path() {
        let mut config = AppConfig::default();
        config.assets_root = PathBuf::from("a");
        config.manifest_path = "b/manifest.json".to_string();
        assert_eq!(config.manifest_full_path(), PathBuf::from("a").join("b/manifest.json"));
    }

    #[test]
    fn test_start_script_full_path() {
        let mut config = AppConfig::default();
        config.assets_root = PathBuf::from("assets_root");
        config.start_script_path = "scripts/main.md".to_string();
        assert_eq!(
    ...
    fn test_validate_fs_assets_root_missing() {
        let mut config = AppConfig::default();
        config.asset_source = AssetSourceType::Fs;
        config.assets_root = unique_temp_path("ring-config-test-no-such-assets");
        config.start_script_path = "start.md".to_string();
    ...
    fn test_validate_fs_script_missing() {
        ...
        config.asset_source = AssetSourceType::Fs;
        config.assets_root = temp_root.path.clone();
        config.start_script_path = "nope.md".to_string();
    ...
    fn test_validate_fs_empty_start_script_path() {
        ...
        config.asset_source = AssetSourceType::Fs;
        config.assets_root = temp_root.path.clone();
        config.start_script_path = "".to_string();
    ...
    fn test_validate_zip_requires_zip_path() {
        let mut config = AppConfig::default();
        config.asset_source = AssetSourceType::Zip;
        config.zip_path = None;
        config.start_script_path = "entry.md".to_string();
    ...
    fn test_validate_zip_missing_zip_file() {
        let mut config = AppConfig::default();
        config.asset_source = AssetSourceType::Zip;
        config.zip_path = Some(unique_temp_path("ring-config-test-missing.zip").to_string_lossy().into());
        config.start_script_path = "entry.md".to_string();
    ...
    fn test_validate_zip_ok() {
        ...
        config.asset_source = AssetSourceType::Zip;
        config.zip_path = Some(zip.to_string_lossy_owned());
        config.start_script_path = "entry.md".to_string();
        assert!(config.validate().is_ok());
    }
    ...
    fn test_config_error_display() {
        let cases = [
            (ConfigError::SerializationFailed("x".to_string()), "配置序列化失败"),
            (ConfigError::IoError("y".to_string()), "配置 IO 错误"),
            (ConfigError::ValidationFailed("z".to_string()), "配置验证失败"),
        ];
