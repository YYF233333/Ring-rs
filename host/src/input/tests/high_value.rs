...
fn test_begin_frame_advances_time() {
    ...
    assert!((manager.state.current_time - 0.016).abs() < 0.001);
    manager.begin_frame(0.033);
    assert!((manager.state.current_time - 0.049).abs() < 0.001);
}
...
fn test_update_click_via_mouse_just_pressed() {
    ...
    manager.state.current_time = 1.0;
    manager.state.last_click_time = 0.0; // 距上次点击超过 0.15s
    manager.state.mouse_just_pressed = true;
...
fn test_debounce_prevents_rapid_click() {
    let mut manager = InputManager::new();
    manager.state.current_time = 0.05; // only 0.05s since start
    manager.state.last_click_time = 0.0; // 0.05 - 0.0 = 0.05 < 0.15, should be blocked
    manager.state.mouse_just_pressed = true;
...
fn test_choice_keyboard_navigation_down() {
    let mut manager = InputManager::new();
    manager.choice.choice_count = 3;
    manager.choice.selected_index = 0;
    manager.state.just_pressed_keys.insert(KeyCode::ArrowDown);

    let result = manager.update(&WaitingReason::WaitForChoice { choice_count: 3 }, 0.016);
    assert!(result.is_none());
    assert_eq!(manager.choice.selected_index, 1);
}
...
fn test_choice_keyboard_navigation_up_saturates() {
    ...
    manager.choice.choice_count = 3;
    manager.choice.selected_index = 0;
    manager.state.just_pressed_keys.insert(KeyCode::ArrowUp);
...
fn test_choice_keyboard_w_s_navigation() {
    let mut manager = InputManager::new();
    manager.choice.choice_count = 3;
    manager.choice.selected_index = 1;
    manager.state.just_pressed_keys.insert(KeyCode::KeyS);

    manager.update(&WaitingReason::WaitForChoice { choice_count: 3 }, 0.016);
    assert_eq!(manager.choice.selected_index, 2);
}
...
fn test_choice_enter_confirms_selection() {
    let mut manager = InputManager::new();
    manager.choice.choice_count = 3;
    manager.choice.selected_index = 2;
    manager.state.current_time = 1.0;
    manager.state.last_click_time = 0.0;
    manager.state.just_pressed_keys.insert(KeyCode::Enter);
...
fn test_choice_mouse_hover_in_rect() {
    ...
    manager.state.mouse_position = (200.0, 220.0);
    let hovered = manager.handle_choice_hover(&rects);
    assert!(hovered);
    assert_eq!(manager.choice.hovered_index, Some(0));
}
...
fn test_choice_mouse_hover_outside_rect() {
    let mut manager = InputManager::new();
    let rects = vec![(100.0, 200.0, 300.0, 50.0)];
    manager.state.mouse_position = (0.0, 0.0);
    let hovered = manager.handle_choice_hover(&rects);
    assert!(!hovered);
...
fn test_choice_mouse_click_selects_hovered() {
    let mut manager = InputManager::new();
    manager.choice.choice_count = 2;
    manager.choice.choice_rects = vec![(0.0, 0.0, 100.0, 50.0), (0.0, 60.0, 100.0, 50.0)];
    manager.state.mouse_position = (50.0, 30.0);
    manager.choice.hovered_index = Some(0);
    manager.state.current_time = 1.0;
    manager.state.last_click_time = 0.0;
    manager.state.mouse_just_pressed = true;
...
fn test_wait_for_click_accepts_single_key_and_hold_repeat_has_boundaries() {
    ...
    let mut manager = InputManager::new();
    manager.state.current_time = 10.0;
    manager.state.pressed_keys.insert(KeyCode::Enter);
    manager.state.hold_timer = 0.29;
    manager.state.last_hold_trigger_time = 0.0;
    assert_eq!(
    ...
    let mut manager = InputManager::new();
    manager.state.current_time = 10.04;
    manager.state.pressed_keys.insert(KeyCode::Enter);
    manager.state.hold_timer = 0.31;
    manager.state.last_hold_trigger_time = 10.0;
    assert_eq!(
...
fn test_wait_for_time_accepts_keyboard_click_and_respects_debounce_boundary() {
    ...
    let mut manager = InputManager::new();
    manager.state.current_time = 0.10;
    manager.state.last_click_time = 0.0;
    manager.state.just_pressed_keys.insert(KeyCode::Space);
    assert_eq!(
        manager.update(
...
fn test_choice_hover_rejects_points_past_right_and_bottom_edges() {
    ...
    manager.state.mouse_position = (200.0, 30.0);
    assert!(!manager.handle_choice_hover(&rects));
    assert!(manager.choice.hovered_index.is_none());

    manager.state.mouse_position = (30.0, 200.0);
    assert!(!manager.handle_choice_hover(&rects));
    assert!(manager.choice.hovered_index.is_none());
}
