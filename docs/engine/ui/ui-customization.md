...
引擎使用数据驱动的 UI 系统。所有布局参数、颜色、字号和图片素材路径均从 `assets/ui/layout.json` 读取。

**配置文件必须存在且包含所有字段**，缺失文件或字段将导致启动报错。仓库自带完整的默认配置文件（对应 1920×1080 基准分辨率），直接修改即可。

> **行为定制**：若需要自定义按钮列表、动作映射、可见性条件或背景切换逻辑，请参阅 [UI 行为定制指南](screens-customization.md)。
...
要修改悬停色和对话框高度，直接编辑 `assets/ui/layout.json` 中对应字段即可：
