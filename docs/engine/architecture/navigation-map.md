...
| `ui/screen_defs/` | `screen_defs.rs` | 数据驱动 UI（screens.json） |
| `ui/layout/` | `layout_config.rs` | 布局配置（layout.json），CSS 变量替代 ScaleContext |
| `egui_screens/game_menu.rs` | `components/game_menu_frame.rs` | 通用菜单框架 |
| `egui_screens/confirm.rs` | `components/confirm_dialog.rs` | 确认弹窗 |
| `ui/toast.rs` | `components/toast.rs` | Toast 提示 |
