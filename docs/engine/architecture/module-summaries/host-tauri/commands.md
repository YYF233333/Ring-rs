...
绝大多数函数接收 `State<AppState>`，lock Mutex 后委托给 `AppStateInner` 方法；少数命令（如 `quit_game`、`update_settings`）还会额外接 `AppHandle` 之类的 Tauri 句柄。需读写 `AudioManager` / `ResourceManager` / `SaveManager` / `AppConfig` 时统一走 `inner.services()` 或 `inner.services_mut()`。
...
| `delete_save` | `client_token, slot` | `()` | 删除存档 |
...
| `update_settings` | `client_token, settings: UserSettings` | `()` | 更新设置（含音量同步与窗口 fullscreen 应用） |
...
- `update_settings` 会立即同步音量到 AudioManager（bgm_volume / sfx_volume 除以 100 转为 0.0–1.0），并把 `settings.fullscreen` 应用到 Tauri 主窗口
...
- `set_host_screen` / `set_playback_mode` 对未知字符串参数不再 fail-open 到默认值，而是显式返回协议错误
