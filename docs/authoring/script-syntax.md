...
在对话文本内部插入等待点，暂停打字机效果：

```markdown
："她缓缓地转过身来。{wait}然后——"
："倒计时：三、{wait 1s}二、{wait 1s}一！"
```

语法：
- `{wait}`：无限等待，需要玩家点击继续（仅跳过当前等待点，不完成全部文本）
- `{wait Ns}`：定时等待 N 秒（N 为正数），到期自动继续

语义约定：
- 标签在解析阶段被提取，对话内容保留纯文本（不含标签）
- 玩家点击时：若处于 `{wait}` 点击等待，仅跳过该等待点继续打字；若处于 `{wait Ns}` 定时等待或正常打字中，完成全部文本
- **Skip 模式**下所有内联等待自动跳过
- 标签名大小写不敏感（`{Wait}`、`{WAIT}` 均可）
...
```markdown
："这是正常速度的文本。{speed 5}这段非常慢。{/speed}恢复正常。"
："正常文本。{speed 2x}这段是两倍速。{/speed}"
```

语法：
- `{speed N}`：设置绝对字速，N 为每秒字符数（正数）
- `{speed Nx}`：设置相对字速，N 为倍率（正数），基于用户设置的基准速度
- `{/speed}`：重置为用户设置的默认速度

语义约定：
- 速度控制仅影响打字机显示速率，不影响文本内容
- 多个 `{speed}` 标签以最后设置的为准
- `{/speed}` 恢复到用户设置面板中的速度值
- 标签名大小写不敏感

### 自动推进修饰符 (-->)

在对话行末尾添加 `-->` 使该行文本显示完成后自动推进到下一条指令，无需玩家点击：
...
语义约定：
- 打字机完成全部文本后立即触发自动推进
- 与 `{wait}` 可组合使用：先等待，再自动推进

### 台词续接 (extend)

将文本追加到当前对话框，不清除已显示的内容：
...
语法：
- `extend "文本内容"`
- 引号内文本支持所有内联标签（`{wait}`、`{speed}`等）
- 支持 `-->` 行尾修饰符

语义约定：
- 新文本追加到当前对话框已有文本之后，打字机从追加位置继续
- 若当前无对话框，`extend` 不产生效果
- 历史记录中，`extend` 文本追加到最近一条对话记录
- 内联标签的字符位置基于追加后的完整文本计算
...
**阶段 25 统一说明**：所有效果的解析逻辑由 Runtime 统一处理，同名效果在不同目标上共享同一份解析与默认值。
