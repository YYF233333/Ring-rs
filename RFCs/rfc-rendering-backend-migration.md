...
- [ ] 在 `host/src/` 新建 `backend/` 模块，封装 wgpu 初始化、窗口、渲染管线
- [ ] 实现 `WgpuBackend` 结构体：device, queue, surface, egui 集成
- [ ] egui 字体初始化：加载 CJK 字体（simhei.ttf）到 `FontDefinitions`，确保中文渲染正常
- [ ] 迁移 `main.rs`：从 `#[macroquad::main]` 改为 winit `EventLoop::run_app`
- [ ] 主循环 `update → draw → present` 骨架就位
- [ ] **门控**：空白窗口 + egui 中文面板可运行

### Phase 1：2D 渲染管线（预计 2-3 天）

- [ ] 实现 textured quad batch renderer（WGSL shader + vertex buffer）
- [ ] 迁移纹理加载：`ResourceManager` 底层从 `Texture2D` 改为 wgpu 纹理
- [ ] 迁移 `TextureCache`：LRU 策略不变，缓存类型替换
- [ ] 迁移 `Renderer::render`：背景、角色绘制通过新管线
- [ ] **门控**：背景图 + 角色立绘正常显示

### Phase 2：Shader 效果迁移（预计 1 天）

- [ ] ImageDissolve shader 从 GLSL 迁移到 WGSL
- [ ] 转场 alpha blend 在新管线中实现
- [ ] 场景转场（`SceneTransitionManager`）适配新渲染后端
- [ ] **门控**：`changeScene` rule 过渡效果正常

### Phase 3：输入系统迁移（预计 0.5 天）

- [ ] `InputManager` 改为消费 winit `WindowEvent`
- [ ] 维护 pressed keys / mouse state
- [ ] `UiContext` 输入源从 macroquad 切换到 winit 事件
- [ ] **门控**：键盘/鼠标交互正常，Skip/Auto 正常

### Phase 4：UI 与屏幕迁移（预计 3-4 天）

- [ ] `host/src/ui/` 全部组件替换为 egui 实现
- [ ] 对话框（`DialogueBox`）+ 打字机效果适配 egui 文本渲染
- [ ] 选项菜单适配 egui
- [ ] `host/src/screens/` 全部页面重写为 egui
  - [ ] Title 页面
  - [ ] Settings 页面（含音量滑块/分辨率切换等）
  - [ ] SaveLoad 页面
  - [ ] History 页面
  - [ ] InGameMenu 页面
- [ ] Toast / Modal 适配 egui
- [ ] **门控**：所有页面可交互；设置可调；存读档正常

### Phase 5：清理与验收（预计 1 天）

- [ ] 移除 `macroquad` 依赖（`host/Cargo.toml`）
- [ ] 移除所有 `use macroquad::` 引用
- [ ] `cargo check-all` 通过
