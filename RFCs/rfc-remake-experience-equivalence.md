...
- [x] **UI 重构（RFC-011）**：季节切换、存读档分页与缩略图、notify/滑块样式、UI 定制文档 已完成
- [ ] UI 风格持续迭代（字体、动效节奏等，可按需推进）
- [x] 保持现有引擎架构清晰，未回退到 Ren'Py 风格耦合实现
...
1. 在 `host-dioxus/src/command_executor.rs` 注册新效果的 Command 处理分支。
2. 在 `host-dioxus/src/render_state.rs` 添加效果状态建模。
3. 在 `host-dioxus/src/vn/` 对应组件中实现渲染逻辑。
...
| RFC-011 | UI 系统后续增强 | Accepted | P2-4 UI 重构已落地（季节切换、存读档增强、视觉细节、定制文档） |
