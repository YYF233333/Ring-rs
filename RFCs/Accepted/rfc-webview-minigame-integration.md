...
- `mini-games` feature gate 正常工作，不影响默认编译
...
- 所有 WebView 相关代码在 `mini-games` feature gate 下条件编译
- 优雅降级：WebView 不可用时给出明确提示，不阻塞 VN 主线
...
- `Cargo.toml`: `wry = { version = "0.49", optional = true }`
- `[features] mini-games = ["wry"]`
- 所有 WebView 代码用 `#[cfg(feature = "mini-games")]` 守护
- `game_mode::is_available()` 运行时查询

### 降级策略

WebView 不可用时：
- `callGame` 命令执行时记录警告日志
- 立即回传 `UIResult` 值为空字符串
- VN 脚本可通过检查结果变量判断是否成功
...
- [ ] `mini-games` feature 启用时 wry 正常编译
- [ ] 不启用 feature 时所有现有功能不受影响
- [ ] GameMode 状态机正确管理 Idle/Running 转换
- [ ] JS Bridge 协议类型定义完整（BridgeRequest / BridgeResponse）
- [ ] `cargo check-all` 通过（不含 mini-games feature）
- [ ] `cargo check -p host --features mini-games` 通过
