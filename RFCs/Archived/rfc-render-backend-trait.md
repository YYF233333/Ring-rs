...
`docs/testing/coverage.md` 明确提出 host 应关注"关键链路的 headless 测试与可测试边界"。但当前以下场景无法在无 GPU 环境下测试：
...
- 更新 `docs/engine/architecture/navigation-map.md`：新增 `rendering_types.rs` 条目
- 更新 `docs/engine/architecture/module-summaries/`：新增或更新 renderer、resources、backend 相关摘要
- 更新 `docs/testing/coverage.md` 中关于 headless 测试边界的描述
